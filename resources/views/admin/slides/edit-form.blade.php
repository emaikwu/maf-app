...
@section('content')
  <form action="/admin/slides/{{$slide->id}}?_method=PUT" method="POST" enctype="multipart/form-data">
    ...
    <input type="text" class="form-control" name="title" id="title" value="{{$slide->title ? $slide->title : old("title")}}">
    </div>
    <div class="form-group">
      <label for="photo">Slide photo <span class="error">{{$errors->first("photo")}}</span></label>
      <input type="file" class="form-control" name="photo" id="photo" value="{{old("photo") ?: $slide->photo }}">
    </div>
    <div class="form-group">
      <label for="info">Slide info <span class="error">{{$errors->first("info")}}</span></label>
      <textarea name="info" id="info" rows="3" class="form-control">{{old("info") ?: $slide->info }}</textarea>
    </div>
    <div class="form-group">
